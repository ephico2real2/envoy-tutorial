...
works on any conforming implementation, not just Envoy Gateway.
...
/          "served_by": "echo-f8fc6d5c9-k9hcn"
/canary/x  "served_by": "canary-5964fbdc9b-wmgfz"
/canaryfoo "served_by": "echo-f8fc6d5c9-k9hcn"
```
...
— and only on a full tie falls back to the older route and then rule order. And
...
x-canary: yes  "served_by": "canary-5964fbdc9b-wmgfz"
x-canary: no   "served_by": "echo-f8fc6d5c9-k9hcn"
```
...
httproute/envoy-13/split/rule/0/backend/0 weight 90 ['10.217.0.196', '10.217.0.197']
httproute/envoy-13/split/rule/0/backend/1 weight 10 ['10.217.0.198']
$ oc get pods -n envoy-13 -l 'app in (echo,canary)' -o custom-columns=NAME:.metadata.name,IP:.status.podIP
NAME                      IP
canary-5964fbdc9b-wmgfz   10.217.0.198
echo-f8fc6d5c9-k9hcn      10.217.0.197
echo-f8fc6d5c9-m9zgz      10.217.0.196
```
...
the split lands so close to exactly 90 / 10. To move more traffic to the canary,
change the weights and apply; nothing restarts.
...
$ sleep 3; oc exec -n envoy-13 client -- curl -s "http://$(oc get gateway eg -n envoy-13 -o jsonpath='{.status.addresses[0].value}')/v2/hello?x=1" | grep -E '"(served_by|path)"'
  "served_by": "canary-5964fbdc9b-wmgfz",
  "path": "/hello?x=1",
...
location: http://192.168.127.101/new/page?x=1
```
...
"served_by": "echo-f8fc6d5c9-m9zgz"
...
6. a route timeout
  /slow answered 504 after 0.498080s
  ✓ /slow -> 504
...
| a more specific rule still loses | it is not more specific by the Gateway API's ranking — for example a header match with a *shorter* path | compare with the ranking in step 2; read the generated order |
